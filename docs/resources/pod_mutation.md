...
- `pod_eviction` (Block List, Max: 1) Eviction settings for enforcement of pod mutations. (see [below for nested schema](#nestedblock--pod_eviction))
...
<a id="nestedblock--pod_eviction"></a>
### Nested Schema for `pod_eviction`

Required:

- `enabled` (Boolean) Whether non-conforming pods are eligible for eviction.
