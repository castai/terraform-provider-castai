...
- `accelerated_networking` (String) Controls SR-IOV accelerated networking on the node NIC. Allowed values: `enabled_if_supported` (default — enable when the VM SKU supports it), `disabled` (force off regardless of SKU capability).
