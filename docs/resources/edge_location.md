...
- `addons` (Attributes List) Optional addons to install on the edge cluster. Defaults to installing nvidia-gpu-operator. Set to an empty list to install no optional addons. (see [below for nested schema](#nestedatt--addons))
...
- `name` (String) Addon identifier. One of: nvidia-gpu-operator, nvidia-dra, nvidia-network-operator, oci-csi.

Optional:

- `values` (String) Helm values for the addon, encoded as a JSON object. Deep-merged over the chart's default values.
