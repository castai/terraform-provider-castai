...
func TestAKSClusterResourceCreateContext(t *testing.T) {
	ctx := context.Background()
	clusterID := "b6bfc074-a267-400f-b8f1-db0850c369b1"

	t.Run("create cluster token handles non-200 response without panic", func(t *testing.T) {
		r := require.New(t)
		mockctrl := gomock.NewController(t)
		mockClient := mock_sdk.NewMockClientInterface(mockctrl)
		provider := &ProviderConfig{
		...
		registerBody := io.NopCloser(bytes.NewReader([]byte(fmt.Sprintf(`{"id": "%s"}`, clusterID))))
		mockClient.EXPECT().
			ExternalClusterAPIRegisterCluster(gomock.Any(), gomock.Any()).
			Return(&http.Response{StatusCode: 200, Body: registerBody, Header: map[string][]string{"Content-Type": {"json"}}}, nil)

		tokenErrorBody := io.NopCloser(bytes.NewReader([]byte(`{"message": "invalid cluster state"}`)))
		mockClient.EXPECT().
			ExternalClusterAPICreateClusterToken(gomock.Any(), clusterID).
			Return(&http.Response{StatusCode: 400, Body: tokenErrorBody, Header: map[string][]string{"Content-Type": {"json"}}}, nil)

		aksResource := resourceAKSCluster()

		diff := map[string]any{
			FieldAKSClusterName:              "test-cluster",
			FieldAKSClusterRegion:            "westeurope",
			FieldAKSClusterSubscriptionID:    "sub-123",
			FieldAKSClusterNodeResourceGroup: "ng-123",
		}
		data := schema.TestResourceDataRaw(t, aksResource.Schema, diff)

		diagnostics := aksResource.CreateContext(ctx, data, provider)

		r.NotEmpty(diagnostics)
		r.True(diagnostics.HasError())
		r.Contains(diagnostics[0].Summary, "creating cluster token")

		r.Empty(data.Get(FieldClusterToken))
		r.Empty(data.Id())
	})

	t.Run("create cluster token handles 500 server error without panic", func(t *testing.T) {
		r := require.New(t)
		mockctrl := gomock.NewController(t)
		mockClient := mock_sdk.NewMockClientInterface(mockctrl)
		provider := &ProviderConfig{
		...
		tokenErrorBody := io.NopCloser(bytes.NewReader([]byte(`{"message": "internal server error"}`)))
		mockClient.EXPECT().
			ExternalClusterAPICreateClusterToken(gomock.Any(), clusterID).
			Return(&http.Response{StatusCode: 500, Body: tokenErrorBody, Header: map[string][]string{"Content-Type": {"json"}}}, nil)
	...
	t.Run("successful cluster creation with token", func(t *testing.T) {
		r := require.New(t)
		mockctrl := gomock.NewController(t)
		mockClient := mock_sdk.NewMockClientInterface(mockctrl)
		provider := &ProviderConfig{
		...
		tokenBody := io.NopCloser(bytes.NewReader([]byte(fmt.Sprintf(`{"token": "%s"}`, expectedToken))))
		mockClient.EXPECT().
			ExternalClusterAPICreateClusterToken(gomock.Any(), clusterID).
			Return(&http.Response{StatusCode: 200, Body: tokenBody, Header: map[string][]string{"Content-Type": {"json"}}}, nil)

		updateBody := io.NopCloser(bytes.NewReader([]byte(`{}`)))
		mockClient.EXPECT().
			ExternalClusterAPIUpdateCluster(gomock.Any(), clusterID, gomock.Any()).
			Return(&http.Response{StatusCode: 200, Body: updateBody, Header: map[string][]string{"Content-Type": {"json"}}}, nil)
		...
		r.Equal(expectedToken, data.Get(FieldClusterToken))
	})
}
